...
end_program = print("Wpisz koniec aby zakończyć program")
maximum_weight = 20
kilograms_sent = 0
parcel_weight = 0
stop_program = False
...
while not stop_program:
    continue_program = input("Czy chcesz kontynować?: ")
    if "tak" == continue_program:
        stop_program = False
        element = float(input("Podaj wagę elementu: "))
        if not element in range(10):
            if element > 10:
                check_elements_weight = element
                print(f"Waga elementu jest większa od 10: ", check_elements_weight)
                parcel_send = 0
                divide = element / 20
                parcel_send += divide
                print("Musisz rozdzielić element i wysłać: ", parcel_send, "paczek")
                stop_program = True
